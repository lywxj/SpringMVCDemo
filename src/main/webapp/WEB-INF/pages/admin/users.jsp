...
<div class="container">
    ...
    <jsp:text>
        Box Perimeter is: ${fuck}
    </jsp:text>

    <c:out value="100+200"/>
    <c:out value="${100*200}"/>
    <c:out value="${fuck}"/>
    <c:if test="${fuck}">11</c:if>
    <c:if test="${!fuck}">22</c:if>

    <c:if test="${empty userList}">数组为空</c:if>
    <c:if test="${!empty userList}">数组不为空</c:if>
    <c:forEach items="${userList}" var="user">${user.id}</c:forEach>
