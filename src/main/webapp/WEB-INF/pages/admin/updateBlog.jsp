...
<div class="container">
    ...
    <hr/>
    <form:form action="/admin/blogs/updateP" method="post" commandName="postP" role="form">
        ...
        <div class="form-group">
            ...
            <select class="form-control" id="userByUserId.id" name="userByUserId.id">
                <c:forEach items="${userList}" var="user">
                    <option value="${user.id}" <c:if test="${user.id==blog.userByUserId.id}">selected="selected"</c:if>>${user.nickname}, ${user.firstName} ${user.lastName}</option>
                </c:forEach>
            </select>
        </div>
        <div class="form-group">
            <label for="content">Content:</label>
            <textarea class="form-control" id="content" name="content" rows="3" placeholder="Please Input Content">${blog.content}</textarea>
        </div>
        <div class="form-group">
